...
interface Papa { id: number; nombre: string; }
...
export function PlanesPadresPage() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([
      api.get<PlanActivo[]>('/planes-papa/activos-hoy'),
      api.get<Papa[]>('/papas'),
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.idPapa || (!form.idServicio && form.tipo === 'SERVICIO') || (!form.idPaquete && form.tipo === 'PAQUETE') || !form.fechaInicio || !form.totalSesiones) return;
    setSending(true);
    setError(null);
    ...
    const q = form.tipo === 'SERVICIO' ? `idPapa=${form.idPapa}&idServicio=${form.idServicio}` : `idPapa=${form.idPapa}&idPaquete=${form.idPaquete}`;
    const nombrePapa = papas.find((n) => n.id === form.idPapa)?.nombre ?? 'el padre';
    const promise = form.tipo === 'SERVICIO' ? api.post('/planes-papa/servicio?' + q, body) : api.post('/planes-papa/paquete?' + q, body);
    promise
      .then(() => {
        load();
        setForm((f) => ({ ...f, idServicio: 0, idPaquete: 0, totalSesiones: 1 }));
        setStep(1);
        setSuccessMessage(`¡Plan asignado con éxito a ${nombrePapa}!`);
  ...
  const selectedPapa = useMemo(() => papas.find(n => n.id === form.idPapa), [papas, form.idPapa]);
  ...
  const filteredPapas = papas.filter(n => n.nombre.toLowerCase().includes(busqueda.toLowerCase()));
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-20">
      <header className="animate-fade-in">
        ...
        <h2 className="text-4xl font-extrabold text-[#111827] tracking-tight">Planes para padres</h2>
        <p className="mt-2 text-sm text-[#4b5563] max-w-2xl">
          Configura planes para padres. Selecciona un padre, elige servicio o paquete y establece la fecha de inicio.
        </p>
      </header>
          ...
          <div className="google-card !p-0 overflow-hidden relative border-none shadow-2xl shadow-indigo-100/20">
            ...
            <div className="p-8 sm:p-10">
              <div className="flex items-center gap-8 mb-10 overflow-x-auto pb-2 scrollbar-none">
                {[
                  { num: 1, label: 'Padre', active: step >= 1 },
                ...
                <div className="animate-fade-in space-y-6">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                    {filteredPapas.map((n) => (
                      <button
                        key={n.id}
                        onClick={() => { setForm(f => ({ ...f, idPapa: n.id })); setStep(2); }}
                        className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all text-left ${form.idPapa === n.id ? 'border-[#2d1b69] bg-indigo-50 shadow-lg shadow-indigo-100/50' : 'border-transparent bg-[#f8f9fa] hover:border-indigo-100 hover:bg-white'}`}
                      >
                        <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-sm ${form.idPapa === n.id ? 'bg-[#2d1b69] text-white' : 'bg-white text-[#2d1b69] border border-indigo-50'}`}>
                          {n.nombre.charAt(0)}
                        </div>
                        <span className="text-sm font-extrabold text-[#111827]">{n.nombre}</span>
                      </button>
                    ))}
                    {filteredPapas.length === 0 && <p className="col-span-full py-10 text-center text-sm text-[#9ca3af] italic">No se encontraron padres con ese nombre.</p>}
                  </div>
                </div>
              )}
                  ...
                  <div className="grid gap-8 sm:grid-cols-2">
                    ...
                    <div className="space-y-4">
                      ...
                      <input
                        ...
                        className="w-full rounded-2xl border-2 border-[#f1f3f4] bg-[#f8f9fa] px-6 py-4 text-sm font-bold focus:border-[#2d1b69] focus:bg-white focus:outline-none transition-all shadow-sm"
                        placeholder="Ej: 5 si el padre ya asistió antes"
                      />
                    </div>
                    <div className="space-y-4">
          ...
          <div className="google-card sticky top-8 !p-8 border-none overflow-hidden shadow-2xl shadow-indigo-100/30">
            ...
            <div className="space-y-6 relative z-10">
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-extrabold text-[#9ca3af] uppercase tracking-wider">Padre</span>
                <span className={`text-sm font-bold transition-all ${selectedPapa ? 'text-[#111827]' : 'text-rose-300 italic'}`}>
                  {selectedPapa ? selectedPapa.nombre : 'Sin seleccionar'}
                </span>
              </div>
